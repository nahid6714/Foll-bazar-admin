...
## Current backend
This Admin uses the Fol Bazar PHP/MySQL API at `https://lakebazar.com/api`. The runtime uses the Laravel API only. Images are uploaded to the website's own `/uploads/products/`, `/uploads/categories/`, or `/uploads/banners/` storage through `/api/upload`.
