...
class Repository(private val api: PhpAdminClient = PhpAdminClient()) {
    ...
    private fun parseBanner(o: JsonObject) = SiteBanner(s(o,"id")?:"",s(o,"banner_type")?:"hero",s(o,"title"),s(o,"alt_text")?:"ফল বাজার ব্যানার",s(o,"image_url")?:"",s(o,"link_url"),s(o,"category_id"),s(o,"category_name"),i(o,"sort_order")?:0,b(o,"is_active")?:true,i(o,"width_percent")?:100,i(o,"height_px")?:180,s(o,"created_at"),s(o,"updated_at"))
    ...
    suspend fun addBanner(bannerType:String,title:String?,altText:String,imageUrl:String,linkUrl:String?,categoryId:String?,sortOrder:Int,widthPercent:Int,heightPx:Int):Result<SiteBanner> = runCatching{withContext(Dispatchers.IO){parseBanner(array(api.post("site_banners",buildJsonObject{put("banner_type",bannerType);put("title",title);put("alt_text",altText.ifBlank{"ফল বাজার ব্যানার"});put("image_url",imageUrl);put("link_url",linkUrl);put("category_id",categoryId);put("sort_order",sortOrder);put("is_active",true)}.toString())).first().jsonObject)}}
    suspend fun updateBanner(b:SiteBanner):Result<SiteBanner> = runCatching{withContext(Dispatchers.IO){parseBanner(array(api.patch("site_banners","id=eq.${b.id}",buildJsonObject{put("banner_type",b.bannerType);put("title",b.title);put("alt_text",b.altText);put("image_url",b.imageUrl);put("link_url",b.linkUrl);put("category_id",b.categoryId);put("sort_order",b.sortOrder);put("is_active",b.active)}.toString())).first().jsonObject)}}
